...
export default function useApplySoundState(sharedState) {
  ...
  useEffect(() => {
    ...
    for (let pad of sharedState.pads) {
      ...
      loadAllBuffers.push(
        decodeAudioFile(context, file).then((decodedBuffer) => {
          newBuffers[pad.soundID] = decodedBuffer;
        })
      );
    }
  ...
  useEffect(() => {
    ...
    for (let [soundID, playbackState] of Object.entries(
      sharedState.playbackState
    )) {
