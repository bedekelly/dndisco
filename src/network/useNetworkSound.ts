...
import {EventData, InitialLoadPayload, LoadPayload, PlayPayload, useClientSocket} from "./useSockets";
import {useEffect} from "react";

export default function useNetworkSound() {
  const { context, unlock, running } = useAudioContext() || {};
  const { playBuffer, loadBuffer, stopBuffer, getLoadedSounds, getVisualizerData, stopAll } = useBuffers("guest");
    ...
    const subscription = subscribe(async ([eventName, event]: [string, EventData]) => {
      ...
      switch (eventName) {
        ...
        case "INITIAL_LOAD": {
          ...
          return sendMessage('LOADED_FILES', [...getLoadedSounds(), ...soundIDs]);
        }
      }
    });
    return () => unsubscribe(subscription);
  }, [context, getLoadedSounds, loadBuffer, playBuffer, sendMessage, stopAll, stopBuffer, subscribe, unsubscribe]);
